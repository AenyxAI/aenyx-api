import { AenyxError, RateLimitError, AuthError, NetworkError } from './utils/errors.js';

const DEFAULT_BASE_URL = 'https://aenyx.onrender.com';
const DEFAULT_TIMEOUT  = 30_000;
const DEFAULT_RETRIES  = 3;

export class AenyxClient {
  /**
    ...
    while (attempt <= this.retries) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new AenyxError(body.error ?? `HTTP ${res.status}`, res.status);
        }

        return await res.json();
      } catch (err) {
        clearTimeout(timer);
        if (err instanceof AenyxError) throw err;
