...
import { AenyxError } from '../utils/errors.js';

const POLL_INTERVAL = 2000;
const POLL_TIMEOUT  = 120_000;

export class ImageModule {
  /** @param {import('../client.js').AenyxClient} client */
  constructor(client) {
    this._client = client;
  }
  ...
  async _poll(jobId, onProgress) {
    ...
    while (Date.now() < deadline) {
      ...
      if (status.status === 'failed') {
        throw new AenyxError(status.error ?? 'Image generation failed');
      }
    }

    throw new AenyxError('Image generation timed out');
  }
